...
comment_id = 4
COMMENTS_URL = f"/api/admin/comments/{comment_id}"
...
def test_update_comment_status__success__returns_200(authenticated_client, status, mocker):
    ...
    mock.assert_called_once_with(mocker.ANY, user_id, comment_id, status)
    mock_clear.assert_called_once()

    assert response.status_code == 200
    assert response.get_json() == {
        "message": f"Коментар {comment_id} змінив статус на {status.value}"
    }
...
def test_update_comment_status__invalid_payload__returns_422(authenticated_client):
    client = authenticated_client(role=UserRole.moderator)
...
def test_update_comment_status__no_payload__returns_400(authenticated_client):
    client = authenticated_client(role=UserRole.moderator)
